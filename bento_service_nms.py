...
from pre_post_processing_nms import PrePostProcessing
...
@artifacts([
    ...
    JSONArtifact('config'),
    BinaryFileArtifact('test_image', file_extension='.png'),
...
class Model_YOLOX_Pack(BentoService):
    ...
    def set_prepost_processing(self):
        if not self.pre_post_processing:
            ...
            self.pre_post_processing = PrePostProcessing(mean=mean, std=std, num_classes=num_classes)
    ...
    def test_data(self, *args): # return some data for running a test
        ...
        # pil_img = Image.open(io.BytesIO(bytearray(self.artifacts.model_test_image)))
