...
class OnnxModelArtifact(BentoServiceArtifact):
    ...
    Args:
        ...
        backend (string): Name of ONNX inference runtime. ["onnxruntime",
                            "onnxruntime-gpu"]
        providers (list): Name of the providers onnxruntime. ['CUDAExecutionProvider', 'CPUExecutionProvider']
    ...
    >>> svc.save()
    """
    def __init__(self, name, backend="onnxruntime", providers=['CUDAExecutionProvider', 'CPUExecutionProvider']):
        super().__init__(name)
        ...
        if self.backend == "onnxruntime" or self.backend == "onnxruntime-gpu":
            ...
            if self._model_proto:
                ...
                return onnxruntime.InferenceSession(
                    self._model_proto.SerializeToString(), providers=self._provider
                )
